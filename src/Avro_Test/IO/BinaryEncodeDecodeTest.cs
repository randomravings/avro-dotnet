...
namespace IO
{
    ...
    public class BinaryEncodeDecodeTest
    {
        ...
        [TestCase(1)]
        [TestCase(-1)]
        [TestCase(-7922816251426433.7593543950335D)]
        [TestCase(7922816251426433.7593543950335D)]
        public void TestDecimal(decimal expected)
        {
        ...
        [TestCase(1, 32)]
        [TestCase(-1, 32)]
        [TestCase(-7922816251426433.7593543950335D, 32)]
        [TestCase(7922816251426433.7593543950335D, 32)]
        public void TestDecimalFixed(decimal expected, int len)
        {
            var bits = decimal.GetBits(expected);
            byte scale = (byte)((bits[3] >> 16) & 0x7F);
            using (var stream = new MemoryStream())
            ...
            using (var decoder = new BinaryDecoder(stream))
            {
                encoder.WriteDecimal(expected, len);
                stream.Seek(0, SeekOrigin.Begin);
        ...
        public void TestDuration(int[] expected)
        {
            ...
            using (var decoder = new BinaryDecoder(stream))
            {
                var expectedDuration = new Tuple<int, int, int>(expected[0], expected[1], expected[2]);
