...
namespace Avro.Test.IO
{
    ...
    {
        [TestCase(0, new byte[] { })]
        public void EncodeNull(int expectedLength, byte[] expectedValue)
        {
        ...
        public void EncodeInt(int value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeLong(long value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeFloat(float value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeDouble(double value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeString(string value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeFixed(byte[] expectedValue, int expectedLength, byte[] value)
        {
            using (var stream = new MemoryStream())
            using (var decoder = new BinaryEncoder(stream))
            {
                decoder.WriteFixed(value);
                Assert.AreEqual(expectedLength, stream.Position, "Encode offset error");
                Assert.AreEqual(expectedValue, stream.GetBuffer().AsSpan(0, expectedLength).ToArray());
            }
        }
        ...
        public void EncodeMap(IDictionary<string, int> value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
                encoder.WriteMap(value, (s, e) => s.WriteInt(e));
                Assert.AreEqual(expectedLength, stream.Position, "Encode offset error");
                Assert.AreEqual(expectedValue, stream.GetBuffer().AsSpan(0, expectedLength).ToArray());
            }
        }
        ...
        public void EncodeMapBlock(IList<IDictionary<string, int>> value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeDecimal(string value, int scale, int expectedLength, byte[] expectedValue)
        {
            var decimalValue = decimal.Parse(value);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
                encoder.WriteDecimal(decimalValue, scale);
                Assert.AreEqual(expectedLength, stream.Position, "Encode offset error");
                Assert.AreEqual(expectedValue, stream.GetBuffer().AsSpan(0, expectedLength).ToArray());
            }
        }
        ...
        public void EncodeDecimalFixed(string value, int scale, int expectedLength, byte[] expectedValue)
        {
            var decimalValue = decimal.Parse(value);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
                encoder.WriteDecimal(decimalValue, scale, expectedLength);
                Assert.AreEqual(expectedLength, stream.Position, "Encode offset error");
                Assert.AreEqual(expectedValue, stream.GetBuffer().AsSpan(0, expectedLength).ToArray());
            }
        }
        ...
        public void EncodeTimeMS(string value, int expectedLength, byte[] expectedValue)
        {
            var time = TimeSpan.ParseExact(value, @"d\:hh\:mm\:ss\.fff", CultureInfo.InvariantCulture);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeTimeUS(string value, int expectedLength, byte[] expectedValue)
        {
            var time = TimeSpan.ParseExact(value, @"d\:hh\:mm\:ss\.ffffff", CultureInfo.InvariantCulture);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeTimeNS(string value, int expectedLength, byte[] expectedValue)
        {
            var time = TimeSpan.ParseExact(value, @"d\:hh\:mm\:ss\.fffffff", CultureInfo.InvariantCulture);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeTimestampMS(string value, int expectedLength, byte[] expectedValue)
        {
            var time = DateTime.ParseExact(value, "yyyy-MM-dd:HH.mm.ss.fff", CultureInfo.InvariantCulture);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeTimestampUS(string value, int expectedLength, byte[] expectedValue)
        {
            var time = DateTime.ParseExact(value, "yyyy-MM-dd:HH.mm.ss.ffffff", CultureInfo.InvariantCulture);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
        ...
        public void EncodeTimestampNS(string value, int expectedLength, byte[] expectedValue)
        {
            var time = DateTime.ParseExact(value, "yyyy-MM-dd:HH.mm.ss.fffffff", CultureInfo.InvariantCulture);
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
                encoder.WriteTimestampNS(time);
                Assert.AreEqual(expectedLength, stream.Position, "Encode offset error");
                Assert.AreEqual(expectedValue, stream.GetBuffer().AsSpan(0, expectedLength).ToArray());
            }
        }
        ...
        public void EncodeUuid(string value, int expectedLength, byte[] expectedValue)
        {
        ...
        [TestCase(null, 1, new byte[] { 0x00 })]
        [TestCase(456, 3, new byte[] { 0x02, 0x90, 0x07 })]
        public void EncodeNullableValue(int? value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
                encoder.WriteNullableValue(value, (s, e) => s.WriteInt(e), 0);
                Assert.AreEqual(expectedLength, stream.Position, "Encode offset error");
                Assert.AreEqual(expectedValue, stream.GetBuffer().AsSpan(0, expectedLength).ToArray());
            }
        }

        [TestCase(null, 1, new byte[] { 0x02 })]
        [TestCase(new byte[] { 0xFF, 0xEE, 0xDD, 0xCC }, 6, new byte[] { 0x00, 0x08, 0xFF, 0xEE, 0xDD, 0xCC })]
        public void EncodeNullableObject(byte[] value, int expectedLength, byte[] expectedValue)
        {
            using (var stream = new MemoryStream())
            using (var encoder = new BinaryEncoder(stream))
            {
                encoder.WriteNullableObject(value, (s, e) => s.WriteBytes(e), 1);
                Assert.AreEqual(expectedLength, stream.Position, "Encode offset error");
                Assert.AreEqual(expectedValue, stream.GetBuffer().AsSpan(0, expectedLength).ToArray());
            }
        }
